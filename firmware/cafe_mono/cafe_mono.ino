...
// ------------------------------------------
void (*pool[])() = {                     // cafe_mono: the duo's ids, only four of them real (the rest = COCO_MOD)
    coco_mod, echo_og, coco_mod, coco_mod, coco_mod, coco_mod, coco_mod, rungler, selfread, coco_mod, coco_mod
};
...
static void pl_load() {
  ...
  (void)b; n = 0;                                  // cafe_mono: always the same four
  { static const int four[4] = {0, 1, 7, 8}; for (int i = 0; i < 4; i++) pl_id[n++] = four[i]; }
...
void pc_line(char *s) {
  ...
  switch (s[0]) {
    case 'P': { char hb[48]; snprintf(hb, sizeof(hb), "HELLO cafe-mono %s %s ota", FW_VERSION, ble_name); pc_out(hb); } break;
    ...
    case 'L': {                            // L = report the playlist · L <id> <id> ... = set it (up to 11 pool ids) and keep it
                ...
                if (false && n > 0) {               // cafe_mono: the list is fixed
