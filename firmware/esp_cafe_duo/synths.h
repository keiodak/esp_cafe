...
void IRAM_ATTR echo_og() {
  ...
  if (preset_mode) {
    ...
    flip_int = FLIPPERAT ? 2000 : 0;
    flip_latch = FLIPPERAT ? true : false;
    skip_int = SKIPPERAT ? 2000 : 0;
    skip_latch = SKIPPERAT ? true : false;
  ...
  for (int i = 0; i < tapsz; i++) {
    ...
    if (wob > 0) {
      ...
      int q0 = myPlacers[i] - oi;
      if (q0 < 0) q0 += myNumbers[i];
      int q1 = q0 - 1;
      if (q1 < 0) q1 += myNumbers[i];
  ...
  // --- SKIP: wobble on/off ---
  if (SKIPPERAT) {
    if (skip_int < 2000) skip_int += 500;
  } else {
    if (skip_int > 0) skip_int -= 50;
  }
  if (skip_int > 1500) {
    if (!skip_latch) {
      skip_latch = true;
  ...
  // --- FLIP: FM depth x1 / x2 ---
  if (FLIPPERAT) {
    if (flip_int < 2000) flip_int += 500;
  } else {
    if (flip_int > 0) flip_int -= 50;
  }
  if (flip_int > 1500) {
    if (!flip_latch) {
      flip_latch = true;
      fm_mode = fm_mode == 1 ? 2 : (fm_mode == 2 ? 0 : 1);
    }
  ...
  if (boot_timer < 8000) {
    boot_timer++;
    if (boot_timer > 4000) initial_earth = e;
  }                    // the resting level
  ...
  {
    uint32_t mask = 0;
    if (pins >= 1) mask |= BIT(12);
    if (pins >= 2) mask |= BIT(13);
    if (pins >= 3) mask |= BIT(14);
    if (pins >= 4) mask |= BIT(15);
    if (pins >= 5) mask |= BIT(16);
    if (pins >= 6) mask |= BIT(17);
    if (pins >= 7) mask |= BIT(21);
    if (pins >= 8) mask |= BIT(22);
    if (pins >= 9) mask |= BIT(26);
    if (pins >= 10) mask |= BIT(27);
    REG(GPIO_OUT_W1TC_REG)
    [0] = YELLOW_MASK & ~mask;
    REG(GPIO_OUT_W1TS_REG)
    [0] = mask;
  }
...
void IRAM_ATTR rungler() {
  ...
  if (pending && xf == 0) {  // never cut a crossfade short
    ...
    if (len < 512) len = 512;
    tail = cur;
    xf = RUNG_XF;  // fade out the old head
    loop_start = a << 14;
    loop_len = len;
    pos = 0;
    cur = loop_start;
  ...
  if (out_bit) REG(GPIO_OUT_W1TS_REG)
               [0] = YELLOW_MASK;
...
struct MoVoice {
  int32_t pq, rate, n, len, edge, wstep, inv;
  bool perc;
  int32_t rq, dr;
};  // rq/dr: MOVE glide, Q20
...
static int32_t grain_tick(uint32_t wpos, int64_t now, bool frz, bool restart) {
  ...
  if (frz && !was_frozen) { anchor = wpos; fpos = mo_last; }            // FREEZE: stop where the playing was
  was_frozen = frz;
  if (mo_sync) {
    mo_sync = false;
    restart = true;
  ...
  if (now >= next_t) {
    ...
    if (mv) {
      ...
      if (rend < 128) rend = 128;
      if (rend > 131072) rend = 131072;
    }
    int32_t span = (int32_t)(((int64_t)len * rate) >> 12);
    int32_t start = -1;
    if (frz) {
      // FREEZE = a stopped play head: every grain starts at the same place (a little spray so it breathes,
      // wider with SCATTER), EARTH scrubs it back and forth. Where / the marks are not used while frozen.
      int32_t spray = 64 + (mo_scat >> 7);                                 // 64 .. ~576 samples
      int32_t off = (int32_t)(((int64_t)((int32_t)mo_rnd(n, 3) - 32768) * spray) >> 15);
      start = (fpos + off - pc_emod * 16) & 0x1FFFF;
    }
    else if (mo_usemarks) {
      int k0 = mo_rnd(n, 6) >> 13;
      for (int k = 0; k < 8; k++)
        if (mo_mark[(k0 + k) & 7] >= 0) {
          start = mo_mark[(k0 + k) & 7];
          break;
        }
    }
    if (start < 0) {
    ...
    int32_t edge = mo_edge;
    if (edge > len / 2) edge = len / 2;
    if (edge < 16) edge = 16;
    v.len = len;
    v.n = len;
    v.edge = edge;
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (v.perc) {  // struck: 32-sample attack, then a curved fall
      if (k < 32) e = k << 7;
        ...
        int32_t lin = (int32_t)(((int64_t)v.n * v.inv) >> 12);
        if (lin > 4096) lin = 4096;
        e = (lin * lin) >> 12;
      }
    } else if (k < v.edge) {
      int32_t ix = (k * v.wstep) >> 16;
      e = mo_win[ix > 128 ? 128 : ix];
    } else if (v.n <= v.edge) {
      int32_t ix = (v.n * v.wstep) >> 16;
      e = mo_win[ix > 128 ? 128 : ix];
    } else e = 4096;
  ...
  if (mo_f < 4096) {
    ...
    if (band > 32767) band = 32767;
    if (band < -32768) band = -32768;
  ...
  {
    static int32_t genv = 0, gg = 0;
    static bool gopen = false;
    int32_t ag = g < 0 ? -g : g;
    if (ag > genv) genv = ag;
    else genv -= (genv >> 10) + 1;
    if (genv < 0) genv = 0;  // peak, held ~30 ms
    if (!gopen && genv > 70) gopen = true;
    else if (gopen && genv < 45) gopen = false;
...
static int32_t dl_tick(int32_t in, int32_t *rout, bool hold) {
  ...
  if (dl_align) {
    ...
    bc = 0;
    dl_click = 300;
  }  // a tap puts the click on the beat
  cl += (dl_tl - cl) >> 11;
  cr += (dl_tr - cr) >> 11;
  ...
  if (tl < (16 << 8)) tl = 16 << 8;
  if (tl > (65000 << 8)) tl = 65000 << 8;
  if (tr < (16 << 8)) tr = 16 << 8;
  if (tr > (65000 << 8)) tr = 65000 << 8;
...
static int32_t nz_osc(int32_t ring, int32_t *side) {
  ...
  uint64_t i64 = ((uint64_t)nz_oinc * bj_exp2(shoct)) >> 16;
  if (i64 > 0x30000000u) i64 = 0x30000000u;  // (under ~6 kHz)
  ...
  int32_t v1 = sn;
  if (a1 > 0) {
    int32_t dr = 4096 + a1 * 8;
    v1 = (int32_t)(((int64_t)nz_tanh((int32_t)(((int64_t)sn * dr) >> 12)) << 11) / nz_tanh(dr >> 1));
  }
  s1 = v1;
  // 2 FOLD (its amount moved by the feedback: WAVE)
  int32_t ad = a2 + (int32_t)((((int64_t)pf * wW) >> 12) >> 1);
  if (ad < 0) ad = 0;
  if (ad > 4096) ad = 4096;
  int32_t v2 = v1;
  if (ad > 0) {
    int32_t fo = nz_fold((int32_t)(((int64_t)v1 * (4096 + ad * 6)) >> 12));
    v2 = v1 + (int32_t)(((int64_t)(fo - v1) * ad) >> 12);
  }
  // 3 RECTIFY
  int32_t v3 = v2;
  if (a3 > 0) {
    int32_t r = (v2 < 0 ? -v2 : v2) * 2 - 2048;
    v3 = v2 + (int32_t)(((int64_t)(r - v2) * a3) >> 12);
  }
...
static int32_t nz_tick(int32_t in, int32_t *rout, bool onebit, bool freeze) {
  ...
  dwrite(wi, na + 2048);
  dwrite(8192 + wi, nb + 2048);
  dwrite(16384 + wi, nc + 2048);
  w++;
  lastc = c;
  ...
  ba += (fq * hl) >> 12;
  lb += (fq * bb) >> 12;
  int32_t hr = xr - lb - ((nz_q * bb) >> 12);
  bb += (fq * hr) >> 12;
  if (la > 32767) la = 32767;
  if (la < -32768) la = -32768;
  if (ba > 32767) ba = 32767;
  if (ba < -32768) ba = -32768;
  if (lb > 32767) lb = 32767;
...
static int32_t co_tick(uint32_t wpos, int32_t in, int32_t rg, bool back) {
  ...
  lph += co_lfo_inc;
  if (co_lfo_depth) {
    int32_t tri = (int32_t)(lph >> 16);
    tri = tri < 32768 ? tri * 2 - 32768 : (65535 - tri) * 2 - 32768;  // -32768..32766
    sp += (int32_t)(((int64_t)sp * tri * co_lfo_depth) >> 27);
  }
  if (back != co_rev) sp = -sp;
  if (sp > 8 * 4096) sp = 8 * 4096;
  if (sp < -8 * 4096) sp = -8 * 4096;
...
void IRAM_ATTR coco_pc() {
  ...
  if (!pc_link) {
    ...
    if (dl_hold || FLIPPERAT || audio_frozen_state || dl_click > 150) {
      LAMP_ON;
    } else {
      LAMP_OFF;
    }
...
void IRAM_ATTR harmony() {
  ...
  if (t == 0) {
    ...
    S = hd_S;
    if (S < 256) S = 256;
    if (S > HD_STRIDE - 2) S = HD_STRIDE - 2;
    for (int k = 0; k < 2; k++) {
  ...
  pout = o;
  int32_t orr = r + 2048;
  if (orr > 4095) orr = 4095;
  if (orr < 0) orr = 0;
  ASHWRITER(orr);

  if (++bc >= (uint32_t)hd_beat) {
    bc = 0;
    click = 300;
  }
  if (click > 0) {
  ...
  pc_wpos = rb * HD_STRIDE + t;
  pc_ppos = ((rb + 2) % 3) * HD_STRIDE + (q[0] >> 12);
  pc_flip = FLIPPERAT ? 1 : 0;
  pc_skip = SKIPPERAT ? 1 : 0;
...
void IRAM_ATTR saturator() {
  ...
  switch (dist_mode) {
    ...
    case 1:
      {
        if (skip_active) {
          lp_mem += (signal - lp_mem) >> 3;
          signal = signal + lp_mem;
        }
        int bias = (cv - 128) * 32;
    ...
    case 4:
      {
        ...
        if (flip_active) {
          ...
          hp_mem += (vinyl_sig - hp_mem) >> 4;
          vinyl_sig = vinyl_sig - hp_mem;
          lp_mem += (vinyl_sig - lp_mem) >> 2;
          vinyl_sig = lp_mem;
        }
...
static inline int32_t IRAM_ATTR fx_lp(int32_t *st, int32_t x, int32_t k) {
  *st += ((x - *st) * k) >> 12;
  return *st;
}
/// the input's history, interpolated: posq = absolute position, Q12
static inline int32_t IRAM_ATTR h_readq(uint32_t posq) {
  uint32_t i = (posq >> 12) & H_MASK;
  int32_t f = (posq >> 4) & 255;
...
static int32_t td_tick(int32_t in, int32_t *rout, bool hold, bool rs) {
  ...
  f = rq & 255;
  a = dread(TD_R + i);
  b = dread(TD_R + ((i + 1) & 0x7FFF));
  int32_t vr = a + (((b - a) * f) >> 8) - 2048;
  if (fill < 40000) fill++;
  if (fill <= (uint32_t)(((tl > tr ? tl : tr) >> 8) + 2)) {
    vl = 0;
    vr = 0;
  }  // the lines still hold old tape
  if (td_clean) {
    lpl = vl;
    lpr = vr;
  }  // ARP_DELAY: no tone filter...
  else {
    fx_lp(&lpl, vl, td_tone);
    fx_lp(&lpr, vr, td_tone);
  }
  hpl += (lpl - hpl) >> 8;
...
static int32_t sm_tick(int32_t in, bool rs) {
  ...
  static int vi = 0;
  static uint32_t ac = 0;
  static int32_t lp = 0;
  static bool ehi = false;
  if (rs) {
    fx_capture = true;
    n[0] = n[1] = 0;
    ac = 0;
...
static int32_t gl_tick(int32_t in, bool hold, bool rs) {
  ...
  if (++bc >= next) {
    ...
    next = gl_ev;
    if ((int32_t)(h3 & 0xFF) < var) {
      static const uint8_t mul[6] = { 1, 2, 3, 4, 1, 1 };
      next = gl_ev * mul[(h3 >> 8) % 6];
      if (((h3 >> 12) & 3) == 0) next = gl_ev / 2;
    }
    if (next < 128) next = 128;
    int32_t ch = gl_chance + fx_em * 400;
    if (ch < 0) ch = 0;  // EARTH = chance
    if ((int32_t)(h & 0xFFFF) < ch && !(hold && left > 0)) {
      ...
      int32_t sl = gl_slice;
      if ((int32_t)((h2 >> 8) & 0xFF) < var) {
        static const int16_t sm[6] = { 64, 96, 128, 256, 384, 512 };
        sl = (sl * sm[(h2 >> 16) % 6]) >> 8;
      }
      if (sl < 64) sl = 64;
      if (sl > 8000) sl = 8000;
      ...
      sst = fx_hw - back;
      rate = 4096;
      if ((int32_t)((h >> 24) & 0xFF) < gl_pitch) {
        static const int32_t pr[5] = { 2048, 8192, 6144, 2731, 3069 };
        rate = pr[(h2 >> 24) % 5];
      }
      pq = 0;
      rr = 0;
  ...
  switch (mv) {
    ...
    case 6:
      {  // CHOP
        ...
        if (ph < 32) g = ph * 8;
        else if (ph < half - 32) g = 256;
        else if (ph < half) g = (half - ph) * 8;
        else g = 0;
        v = (in * g) >> 8;
      }
      break;
    case 7:
      {  // CRUSH
        ...
        if (++hn >= hl) {
          hn = 0;
          held = in & ~((1 << (4 + (gl_crush >> 3))) - 1);
        }
        v = held;
      }
      break;
    default:
      {  // STUTTER · BACKWARDS · RATCHET · PITCH STEPS · SCATTER
        int32_t off = pq >> 12;
        pq += rate;
        ...
        int32_t e = off < 32 ? off : (cur - off < 32 ? cur - off : 32);
        if (e < 0) e = 0;
        v = (v * e) >> 5;
        if (gl_crush) {
          if (++hn >= 1 + (gl_crush >> 2)) {
...
static int32_t fo_tick(int32_t in, bool rs) {
  ...
  if (!pos && lpd > 40) {
    pos = true;
    ff = -ff;
  } else if (pos && lpd < -40) pos = false;
  int32_t down = ff * env;
  int32_t up = (a - env) * 2;
  int32_t x = in + ((down * fo_down) >> 8) + ((up * fo_up) >> 8);
  int32_t drv = fo_drive + fx_em * 12;
  if (drv < 64) drv = 64;  // EARTH = drive
  x = ((x * drv) >> 8) + fo_bias;
  int32_t tf = (x + 2048) & 8191;
  if (tf >= 4096) tf = 8191 - tf;
  x = tf - 2048;
  ...
  int32_t g = (env * 3) >> 3;
  if (g < 40) g = 40;
  if (g > 110) g = 110;  // as loud as the input, not full scale
  x = (x * g) >> 8;
...
static int32_t sd_tick(int32_t in, int32_t *rout, bool hold, bool rs) {
  ...
  ph += sd_lfo;
  int32_t tri = (int32_t)(ph >> 16);
  tri = tri < 32768 ? tri - 16384 : 49151 - tri;  // -16384..16383
  ...
  if (tl < (8 << 8)) tl = 8 << 8;
  if (tl > (4090 << 8)) tl = 4090 << 8;
  if (tr < (8 << 8)) tr = 8 << 8;
  if (tr > (4090 << 8)) tr = 4090 << 8;
  ...
  f = rq & 255;
  a = dread(SD_R + i);
  b = dread(SD_R + ((i + 1) & 0xFFF));
  int32_t vr = a + (((b - a) * f) >> 8) - 2048;
  if (fill < 5000) fill++;
  if ((int32_t)fill <= (tr >> 8) + 2) {
    vl = 0;
    vr = 0;
  }  // the lines hold old sound at first
  // attacks in the input pluck the string: a noise burst one period long
  int32_t ai = in < 0 ? -in : in;
  ef += (ai - ef) >> 4;
  es += (ai - es) >> 11;
  if (cool > 0) cool--;
  if (sd_pluck > 0 && cool == 0 && ef > es * 2 + 120) {
    burst = tl >> 8;
    cool = 2205;
  }
  int32_t nz = 0;
...
static inline void fx_run(int e, int32_t in, int32_t *l, int32_t *r, bool hold) {
  ...
  switch (e) {
    case 0:
      {
        int32_t g = cl_g + ((cl_g * fx_em) >> 7);
        if (g < 0) g = 0;  // EARTH = level (VCA)
        *l = *r = (in * g) >> 8;
      }
      break;
    case 1:
...
void IRAM_ATTR multi() {
  ...
  static int32_t xf = 0, xfl = 1;
  static bool last_frozen = false;
  static int flip_i = 0, skip_i = 0;
  static bool flip_l = true, skip_l = true;
  static uint32_t rng_n = 0, bc = 0;
  static int click = 0;
  ...
  if (preset_mode) {
    ...
    flip_i = FLIPPERAT ? 2000 : 0;
    flip_l = flip_i > 0;
    skip_i = SKIPPERAT ? 2000 : 0;
    skip_l = skip_i > 0;
    blinks = (cur + 1) * 2;
    bt = 0;
  }
  ...
  if (FLIPPERAT) {
    if (flip_i < 2000) flip_i += 500;
  } else {
    if (flip_i > 0) flip_i -= 50;
  }
  if (flip_i > 1500) {
    if (!flip_l) {
      flip_l = true;
      if (open) {
        fx_want = (fx_want + 1) % FX_N;
        since = 0;
        open = false;
      }
    }
  } else if (flip_i < 100) flip_l = false;
  if (SKIPPERAT) {
    if (skip_i < 2000) skip_i += 500;
  } else {
    if (skip_i > 0) skip_i -= 50;
  }
  if (skip_i > 1500) {
    if (!skip_l) {
      skip_l = true;
      if (open) {
        fx_want = (fx_want + 1 + (int)(mo_hash(fx_seed + rng_n * 0x632BE5ABu) % (FX_N - 1))) % FX_N;
        rng_n++;
        since = 0;
      }
    }
  ...
  bool bchg = audio_frozen_state != last_frozen;
  last_frozen = audio_frozen_state;
  ...
  t = fx_hw & H_MASK;
  pc_wpos = fx_hw & H_MASK;
  pc_ppos = fx_hw & H_MASK;
  pc_flip = FLIPPERAT ? 1 : 0;
  pc_skip = SKIPPERAT ? 1 : 0;
